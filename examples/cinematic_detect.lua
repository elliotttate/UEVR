...
-------------------------------------------------------------------------------
-- Show / Hide Cut-Scene Character.
-------------------------------------------------------------------------------
local function Hide_C_Mesh()
	...
	if skeletal_mesh_c ~= nil then
		...
		for i, mesh in ipairs(skeletal_meshes) do
			if mesh:get_fname():to_string() == "Additional_Top" and string.find(mesh:get_full_name(), "Max") then
				mesh:call("SetRenderInMainPass", false)
				mesh:call("SetVisibility", false)
			end
			if mesh:get_fname():to_string() == "Additional_Head" and string.find(mesh:get_full_name(), "Max") then
				mesh:call("SetRenderInMainPass", false)
				mesh:call("SetVisibility", false)
			end
		end
...
-------------------------------------------------------------------------------
-- Show / Hide Main Playing Character.
-------------------------------------------------------------------------------
local function Hide_Mesh()
	local wpawn = api:get_local_pawn(0)
	local wparts = wpawn.AdditionalMeshes

	if wparts ~= nil then
		for i, mesh in ipairs(wparts) do
			if string.find(mesh:get_full_name(), "Head") then
				mesh:call("SetRenderInMainPass", false)
				mesh:call("SetVisibility", false)
			end
			if string.find(mesh:get_full_name(), "Top") then
				mesh:call("SetRenderInMainPass", false)
				mesh:call("SetVisibility", false)
			end
		end
...
uevr.sdk.callbacks.on_xinput_get_state(function(retval, user_index, state)
	...
	if pawn ~= nil then
		...
		if (pawn_name ~= nil) then
			...
			if (string.find(pawn_name, "ChronosCameraPawn_C")) or (string.find(pawn_name, "BP_Photography")) then
				Show_Mesh()
				Show_C_Mesh()
